...
# Log file setup
log_filename = datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + "_gnss.log"
log_file = open(log_filename, 'w')
print(f"Logging all GNSS data to: {log_filename}")
...
try:
    while True:
        ...
        if raw_line:
            try:
                # Log raw line with timestamp
                timestamp = datetime.now().strftime('%H:%M:%S.%f')[:-3]
                decoded_line = raw_line.decode('ascii', errors='replace').rstrip('\r\n')
                log_file.write(f"{timestamp} {decoded_line}\n")
                log_file.flush()  # Ensure timely write
                
                # For parsing, use stripped version
                ...
                timestamp = datetime.now().strftime('%H:%M:%S.%f')[:-3]
                log_file.write(f"{timestamp} DECODE_ERROR: {raw_line}\n")
                log_file.flush()
